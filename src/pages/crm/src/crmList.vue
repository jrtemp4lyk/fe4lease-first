...
<div id="customerList">
  ...
  <div id="search-container">
    <!-- <div class="fl">
      <el-button type="primary" @click="addEnterprise(0)">添加</el-button>
    </div> -->
    <div class="fr">
...
export default {
  data() {
    return {
      ...
      pkTemp: '8e38d372-5159-4f6c-a2e1-3bd3f648c079',
      currentValue: '',
      isHide: true,
      searchStr: {
        name: ''
  ...
  methods: {
    ...
    //   } else if(type === 1) {
    //     this.$router.push('/crm/personalInfo/add');
    //   } else if(type === 2) {
    //     this.$router.push('/crm/enterpriseInfo/add');
    //   }
    // },
    // 点击查询按钮触发
    search() {
    ...
    editTableRow(row) {
      if(row.type === 1 || row.type === "1") { // 企业客户
        this.$router.push('/crm/enterpriseInfo/' + row.id);
      } else if(row.type === 2 || row.type === "2") { // 个人客户
        this.$router.push('/crm/personalInfo/' + row.id);
      } else {
...
<style>
  ...
  #customerList .content_main {
    background-color: #ffffff;
    min-height: 350px;
    padding-bottom: 20px;
    margin-top: 20px;
  }
  #customerList .yon-uitemplate {
    margin: 20px;
  }

  /* 重置表格字体及表头背景，参考友人才 */
  #customerList .el-table__header th{
    font-size: 15px;
    font-weight: 400;
    color: #333;
  }
  #customerList .el-table__body .cell{
    font-size: 13px;
    font-weight: normal;
    color: #333;
  }
  #customerList .el-table thead tr, #customerList .el-table thead tr th {
    background-color: #eee;
  }
  #customerList .el-table__footer-wrapper thead div, #customerList .el-table__header-wrapper thead div {
    background-color: #eee;
    color: #333;
  }
